    ...
    <div className="pb-24">
        ...
        {data?.referral_code ? (
          <>
            <div className="flex items-center gap-2 mb-4">
              <div className="flex-1 font-mono text-lg font-bold text-center py-3 bg-white/5 rounded-xl text-lime-400 tracking-wider">{data.referral_code}</div>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="glass-card rounded-2xl p-4">
        ...
        <div className="flex gap-2">
          <input value={applyCode} onChange={e => setApplyCode(e.target.value.toUpperCase())} placeholder="Enter code" className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-lime-500/50" data-testid="apply-referral-input" />
        ...
        {applyMsg && <p className="text-xs text-lime-400 mt-2">{applyMsg}</p>}
      </motion.div>
    </div>
  );
}
