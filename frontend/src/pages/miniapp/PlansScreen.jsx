      ...
      <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="pb-24">
        ...
        <div className="glass-card rounded-2xl p-4 mb-4">
          ...
          <div className="flex gap-2">
            <input type="text" value={couponCode} onChange={e => setCouponCode(e.target.value.toUpperCase())} placeholder="COUPON CODE" className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-lime-500/50" data-testid="coupon-input" />
        ...
        <AnimatePresence>
          {showManualSheet && (
            <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} className="fixed inset-0 z-50 flex items-end" onClick={e => e.target === e.currentTarget && setShowManualSheet(false)}>
              ...
              <motion.div className="relative w-full max-h-[85vh] overflow-y-auto rounded-t-3xl p-5 pb-8" style={{ background: "hsl(0, 0%, 5%)", border: "1px solid hsl(0, 0%, 12%)" }}>
                ...
                {uploadStep === "result" ? (
                  ...
                  <div className="text-center">
                    ...
                    {screenshotPreview ? (
                      ...
                      <button onClick={() => fileInputRef.current?.click()} className="w-full py-10 border-2 border-dashed border-white/10 rounded-2xl text-zinc-500 hover:border-lime-500/30 hover:text-zinc-300 transition-all mb-4" data-testid="select-screenshot-btn">
                  ...
                  <>
                    ...
                    <div className="glass-card rounded-2xl p-4 mb-4 text-center">
                      ...
                      {qrLoading ? (
                        <div className="py-8"><Loader2 className="w-8 h-8 animate-spin text-lime-400 mx-auto" /></div>
  ...
  return (
    <div className="pb-24">
        ...
        {plans.map((plan, i) => {
          ...
          return (
            <motion.div
              ...
              className={`glass-card rounded-2xl p-4 cursor-pointer active:scale-[0.98] transition-all relative ${isPopular ? "border-lime-500/30 ring-1 ring-lime-500/20" : ""}`}
      ...
      <div className="mt-6 text-center">
        <p className="text-xs text-zinc-600">Trusted by <span className="text-lime-400 font-semibold">1000+</span> members</p>
      </div>
    </div>
  );
}
