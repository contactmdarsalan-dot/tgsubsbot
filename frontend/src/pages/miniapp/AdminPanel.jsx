  ...
  return (
    <div className="pb-24">
      <div className="flex items-center gap-3 mb-5">
        <div className="w-10 h-10 rounded-xl bg-lime-500/20 flex items-center justify-center"><Sparkles className="w-5 h-5 text-lime-400" /></div>
        <div>
      ...
      <div className="flex gap-1.5 mb-5 overflow-x-auto pb-1">
        {sections.map(s => {
          const Icon = s.icon;
          return (
            <button key={s.id} onClick={() => setActiveSection(s.id)} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold whitespace-nowrap transition-all ${activeSection === s.id ? "bg-lime-500/20 text-lime-400 border border-lime-500/30" : "bg-white/5 text-zinc-500 border border-transparent"}`} data-testid={`admin-tab-${s.id}`}>
              <Icon className="w-3.5 h-3.5" />{s.label}
              {s.badge > 0 && <span className="px-1.5 py-0.5 text-[10px] bg-lime-500 text-white rounded-full">{s.badge}</span>}
            </button>
          );
        })}
      </div>
        ...
        <>
          ...
          {activeSection === "broadcast" && (
            <div className="glass-card rounded-2xl p-5">
              ...
              <textarea value={broadcastMsg} onChange={e => setBroadcastMsg(e.target.value)} placeholder="Type your message..." rows={4} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-zinc-600 resize-none focus:outline-none focus:border-lime-500/50 mb-3" data-testid="broadcast-input" />
