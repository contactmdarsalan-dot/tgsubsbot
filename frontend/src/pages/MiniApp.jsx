...
export default function MiniApp() {
  ...
  useEffect(() => {
    if (tg) { tg.ready(); tg.expand(); tg.setHeaderColor("#05050A"); tg.setBackgroundColor("#05050A"); }
    initApp();
  }, []);
  ...
  if (phoneScreen) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6" style={{ background: "#05050A" }}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-zinc-500">Enter your phone to unlock <span className="text-indigo-400 font-semibold">20% OFF</span></p>
          </div>

          <div className="glass-card rounded-2xl p-5 mb-4">
            <input type="tel" value={phoneNum} onChange={e => setPhoneNum(e.target.value.replace(/\D/g, "").slice(0, 10))} placeholder="Enter phone number" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3.5 text-white text-center text-lg font-mono placeholder-zinc-600 focus:outline-none focus:border-indigo-500/50 mb-4" data-testid="phone-input" />
  ...
  return (
    <MiniAppContext.Provider value={contextValue}>
      <div className="min-h-screen text-white relative" style={{ background: "#05050A" }}>
        {/* Discount popup */}
        <AnimatePresence>
          {discountPopup && (
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="fixed top-4 left-4 right-4 z-50 glass-card rounded-2xl p-4 text-center border-indigo-500/30">
          ...
          {activeTab === "notifications" && (
            <div className="pb-24">
              ...
              {notifications.length === 0 ? (
                ...
                <div key={n.id} className={`glass-card rounded-xl p-3 mb-2 border-l-2 ${n.type === "warning" ? "border-amber-500" : n.type === "promo" ? "border-indigo-500" : "border-cyan-500"}`}>
        ...
        {/* Bottom Nav */}
        <nav className="fixed bottom-0 w-full z-40 pb-safe" style={{ background: "rgba(5,5,10,0.9)", backdropFilter: "blur(20px)", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          <div className="flex items-center justify-around px-2 py-2">
            {mainTabs.map(tab => {
              const Icon = tab.icon;
              const isActive = activeTab === tab.id;
              return (
                <button key={tab.id} onClick={() => switchTab(tab.id)} className={`flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl transition-all ${isActive ? "text-indigo-400" : "text-zinc-600"}`} data-testid={`nav-${tab.id}`}>
            ...
            <div className="relative">
              <button onClick={() => setMoreOpen(!moreOpen)} className={`flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl transition-all ${moreTabs.some(t => t.id === activeTab) ? "text-indigo-400" : "text-zinc-600"}`} data-testid="nav-more">
              ...
              <AnimatePresence>
                {moreOpen && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute bottom-full right-0 mb-2 w-48 rounded-2xl p-2 z-50" style={{ background: "#0D0D14", border: "1px solid rgba(255,255,255,0.1)" }}>
                    {moreTabs.map(tab => {
                      const Icon = tab.icon;
                      return (
                        <button key={tab.id} onClick={() => switchTab(tab.id)} className={`flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-sm transition-all ${activeTab === tab.id ? "bg-indigo-500/15 text-indigo-400" : "text-zinc-400 hover:text-white hover:bg-white/5"}`} data-testid={`nav-${tab.id}`}>
